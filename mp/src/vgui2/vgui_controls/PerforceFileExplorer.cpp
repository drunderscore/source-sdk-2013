...
void PerforceFileExplorer::PopulateFileList()
{
	...
	{
		const char *pFileName2 = p4->String( fileList[i].m_sLocalFile );
		if ( !pFileName2[0] )
			continue;

		int nItemID = m_pFileList->FindFile( pFileName2 );
		...
		if ( nItemID == m_pFileList->InvalidItemID() )
		{
			...
			bFileExists = false;
			nItemID = m_pFileList->AddFile( pFileName2, false, fileList[i].m_bDir );
		}
