...
void CKeyBindingHelpDialog::PopulateList()
{
	...
	while ( pPanel->IsKeyBindingChainToParentAllowed() )
	{
		...
		while ( map )
		{
			...
			if ( k == c )
			{
				int k2 = maps.AddToTail( );
				maps[k2].m_pMap = map;
				maps[k2].m_pPanel = pPanel;
			}
			map = map->baseMap;
		}
	...
	for ( i = 0; i < c; ++i )
	{
		...
		for ( j = 0; j < bindings; ++j )
		{
			...
			// Find the binding
			KeyBindingMap_t *bindingMap = pPanel2->LookupBinding( kbMap->bindingname );
			if ( bindingMap && 
		...
		for ( j = 0; j < mappings; ++j )
		{
			...
			pPanel2->LookupBoundKeys( kbMap->bindingname, list );
			if ( list.Count() > 0 )
				continue;
